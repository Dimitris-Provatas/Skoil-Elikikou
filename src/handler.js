            ...
            )
        {
            ...
            console.log(
                bot.guilds.array().forEach(guild => {
                    bot.guilds.get(guild.id).fetchMembers().then(res => {
            ...
            try {
                targetId = Object.keys(bot.users).filter(u => u.username === payload.target).id;
            }
            catch (err) {
                console.log(`O ${message.author.tag} μου ζήτησε να βρω τον ${payload.target} και δεν τον βρήκα.`);
                console.log("----------------------------------------------------------------------------------------------------------------------------");
            }
            ...
            if (targetId)
            {
                ...
                .then(
                    ...
                    async error =>
                    {
                        console.log(error);
                        console.log("----------------------------------------------------------------------------------------------------------------------------");
                        await message.author.send(`Κάτι πήγε στραβά και δεν μπόρεσα να παραδώσω το μήνυμα! Το πρόβλημα ήταν: \`\`\`${error.name}: ${error.message}\`\`\``);
                    }
                );
            }
            else message.author.send("Δεν βρήκα τον στόχο. Πρέπει να είναι κάποιος που να έχω τουλαχιστον έναν σέρβερ κοινό!");
        }
    }
}

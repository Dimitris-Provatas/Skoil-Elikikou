...
function GetMentions(bot, msg) {
  ...
  while (mentions.includes(bot.user.id)) {
    if (mentions.length === 1) mentions = [];
    else mentions = mentions.splice(mentions.indexOf(bot.user.id) - 1, 1);

    if (mentions.length > 0)
      mention = msg.mentions.users.get(mentions[mentions.length - 1]);
    else mention = msg.author;
  }
...
function Suggestion(suggestion, author, server) {
  ...
  fs.appendFile(
    suggestionsFile,
    `${time}: O \'${author}\' από τον σέρβερ \'${server}\' προτίνει: ${suggestion}\r\n`,
    function (err) {
      if (err) console.log(err);
      else console.log(`${time}: O ${author} έκανε μία πρότασή!`);
...
function ConsoleError(type, user, server, channel) {
  const time = GetTime();
  console.log(
    `${time}: No action taken for ${type} ${user} on server \'${server}\' in channel \'${channel}\'!`,
  );
  console.log(
    `----------------------------------------------------------------------------------------------------------------------------`,
  );

...
async function PerformCommands(bot, message) {
  ...
  if (command.indexOf("{") !== -1 && command.indexOf("}") !== -1) {
    await message.channel.send(
      `${message.author} ξέρω τι πας να κάνεις εκεί, κακό! Kρίμα που τώρα οι <@&488730147894198273> θα μάθουν τι πήγες να κάνεις!`,
    );
    return;
  }
  ...
  if (command.startsWith("play ")) {
    ...
    eval(
      "try { play(bot, message, dir, file); } catch(err) { console.log(err) }",
    );
  } else {
    command = command.split(" ");
    ...
    try {
      ...
      await message.channel.send(
        `Δεν υπάρχει αυτό το command ${message.author}! Για να δεις τις εντολές, γράψε: \`\`\`skoil help\`\`\``,
      );
      console.log(err);
    }
  }
...
async function play(bot, message, dir, file) {
  ...
  if (!player.voiceChannel) {
    await message.channel.send(
      `Δεν είμαι σε κάποιο voice channel ${message.author}! Για να έρθω στο κανάλι που είσαι, γράψε: \`\`\`skoil join\`\`\``,
    );
    return;
  }
  ...
  if (!fs.existsSync(target)) {
    await message.channel.send(
      `Δεν υπάρχει αυτό το αρχείο ${message.author}! Για να δεις τα αρχεία, γράψε: \`\`\`skoil files <folder_name?>\`\`\``,
    );
    return;
  }
...
async function playSound(message, target, dir, file) {
  ...
  player.connection.play(target).on("finish", () => {
    ...
    if (player.playQueue.length > 0)
      playSound(
        message,
        player.playQueue[0].target,
        player.playQueue[0].dir,
        player.playQueue[0].file,
      );
  });
}
...
async function prune(bot, message, number) {
  ...
  if (number === 69 || number === 420 || number === 69420) {
    await message.channel.send(
      `LOL, NICE, μου αρέσει ο τρόπος που σκέφτεσαι ${message.author}! Αλλά δεν μπορώ να το κάνω αυτό. Oopsie, αλλά λολ και λεφμάο!`,
    );
    return;
  }
